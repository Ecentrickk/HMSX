...
interface TimelineEvent {
  id: string;
  date: Date;
  type: 'APPOINTMENT' | 'REPORT' | 'HISTORY' | 'DIAGNOSIS';
  title: string;
  subtitle: string;
  notes?: string;
  status?: string;
...
export function PatientTimeline({ appointments = [], reports = [], medicalHistories = [], diagnoses = [] }: { appointments?: any[], reports?: any[], medicalHistories?: any[], diagnoses?: any[] }) {
  ...
  const events: TimelineEvent[] = [
    ...
    ...diagnoses.map((d: any) => ({
      ...
      title: `Diagnosis: ${d.icdCode || 'Clinical'}`,
      subtitle: d.diagnosisString,
      notes: d.notes,
      status: 'CONFIRMED'
    }))
      ...
      {events.map((event, index) => (
        <div key={event.id} style={{ display: 'flex', gap: 'var(--space-4)' }}>
          ...
          <div style={{ 
            ...
            {event.notes && (
              <div style={{ marginTop: 'var(--space-2)', padding: 'var(--space-2)', background: 'rgba(0,0,0,0.03)', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-xs)', fontStyle: 'italic', color: 'var(--text-muted)' }}>
                "{event.notes}"
              </div>
            )}
